...
public class SystemClass {
	...
	//――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――― bs-pvo : row카운팅과 페이지 셋팅
		public static PageVO countPage(PageVO pvo, int selectNum,BodService bs, RepService rs) {
			...
			pvo.setP_start(pvo.getP_curpage() * pvo.getCri().getAmount());
			
			//끝나는row번호
			//1페이지의 경우 start=10 이므로 10 - 10 = 0
			//row 0번부터 10번까지 선택
			pvo.setP_end(pvo.getP_start() - pvo.getCri().getAmount()-1);
			...
			totalPage = pvo.getP_totrow() / pvo.getCri().getAmount();
			if(pvo.getP_totrow() % pvo.getCri().getAmount() != 0) { 
				totalPage++; 
			}	
			//5.pvo에 전체 페이지 개수 셋
